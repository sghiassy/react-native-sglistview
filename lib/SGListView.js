...
var SGListView = React.createClass({
  ...
  handleScroll: function(evt) {
    ...
    this.layoutTree.queryInterval( yt1, yt2, (interval) => {
      var cell = interval.cell

      if( interval[1] < y1 || interval[0] > y2 ) {
        // Then it's in the enlarged interval but outside the current window:
        if( cell.state.visibility===true ) cell.setVisibility(false)
      } else {
        // Otherwise overlaps the currently visible interval:
        if( cell.state.visibility===false ) cell.setVisibility(true)
      }
  ...
  checkVisibility: function(interval) {
    ...
    // Expand the interval so that things are rendered before they come on screen.
    // Not DRY, but no need to overcomplicate this.
    y1 -= this.props.preemptiveLoadDistance
    y2 += this.props.preemptiveLoadDistance

    return interval[0] < y2 && interval[1] > y1
  },
